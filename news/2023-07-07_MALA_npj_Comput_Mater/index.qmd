---
title: "Machine learning takes materials modeling into new era"
meta:
  - name: description
    content: "News and updates from the Machine Learning for Materials Design (ML4MD) Lab: Machine learning takes materials modeling into new era."
  - name: keywords
    content: "ML4MD Lab, News, Machine Learning, Materials Modeling, MALA"
#description: " "
#author: " "
date: "2023-07-07"
image: MALA_npj_2023.webp # Ensure this image has appropriate alt text if used directly in content
draft: false
categories:
  - scientific highlight
format:
  html:
    page-layout: full
    toc: false
---
![](MALA_npj_2023.webp){.news-thumbnail alt="Image for Machine learning takes materials modeling into new era"}

The arrangement of electrons in matter, known as electronic structure, plays a critical role in fundamental and applied research such as drug design and energy storage. However, the lack of a simulation technique that provides both high fidelity and scalability across different time and length scales has long been an obstacle to the progress of these technologies. We have developed a machine learning method – the Materials Learning Algorithms (MALA) – that replaces traditional electronic structure simulation techniques. MALA enables electronic structure simulations at previously unattainable length scales.

Read more about this in the [press release](https://www.hzdr.de/db/Cms?pOid=69680&pNid=3438){target="_blank"}.
